...
def _tag_ltx_hdr_color_space(item):
    attempts = (
        ("ACES Input Transform", LTX_HDR_INPUT_TRANSFORM),
        ("Input Color Space", LTX_HDR_INPUT_TRANSFORM),
        ("Input Colorspace", LTX_HDR_INPUT_TRANSFORM),
        ("Input Color Space", "sRGB Linear"),
        ("Input Colorspace", "sRGB Linear"),
        ("Input Gamma", "Linear"),
        ("Gamma", "Linear"),
    )
    accepted = []
    for key, value in attempts:
        try:
            if item.SetClipProperty(key, value):
                accepted.append(key)
        except Exception:
            pass
    return accepted
...
def _import_exr_sequence(project, exr_dir, name, metadata=None):
    ...
    accepted_color_tags = _tag_ltx_hdr_color_space(item)
    try:
        comments = "Generated by LTX HDR Resolve v1 from " + name + "\n" + LTX_HDR_COLOR_NOTE
        if accepted_color_tags:
            comments += "\nResolve clip color tag attempts accepted: " + ", ".join(accepted_color_tags)
        else:
            comments += "\nResolve did not report an accepted script color tag; set the ACES Input Transform manually if needed."
        if metadata:
            comments += "\n" + metadata
